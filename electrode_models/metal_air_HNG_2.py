...
class electrode(): 
    ...
    def __init__(self, input_file, inputs, sep_inputs, counter_inputs,    
        ...
        self.SVptr['Histogram'] = np.arange(3 + self.elyte_obj.n_species, 3 + self.elyte_obj.n_species + self.n_bins +1)
    ...
    def initialize(self, inputs, sep_inputs):
        ...
        SV[self.SVptr['histogram']] = self.radius
    ...
    def residual(self, t, SV, SVdot, sep, counter, params):
        ...
        dhistogram_dt = np.zeros_like(SV['histogram'])
        ...
        a_d = (ck_elyte['LiO2[elyt]']*ct.avogadro)**(-1./3.) # length scale of diffusion
        r_crit = 2.*self.gamma_surf*V/(ct.gas_constant*params['T']*math.log(ck_elyte['LiO2[elyt]']/self.c_liO2_sat*ck_elyte['Li+[elyt]']/self.c_li_sat))  # m // critical radius
        ...
        N_sites = SV[SVptr['Area']]/(math.pi*r_crit**2) # number of nucleation sites
        k_nuc= self.D_LiO2*(a_d**-2)

        DN_Dt = k_nuc*N_sites*Z*math.exp(-Del_G_Crit/(ct.boltzmann*params['T']))
        ...
        for i, N in enumerate(Histogram):
            Dr_dt[i] = self.D_LiO2*V*(ck_elyte['Li+[elyt]']- self.c_li_sat)*(ck_elyte['LiO2[elyt]']
        ...
        sdot_elyte_c[self.index_Li] -= i_dl / ct.faraday - DN_Dt*V_crit/(V*self.dy*self.dy) - 2.*np.sum(dhistogram_dt*Dr_dt,self.radius
            *self.radius)*np.pi*V_crit/(V*self.dy*self.dy)
        sdot_elyte_c[self.index_LiO2] -= i_dl / ct.faraday - DN_Dt*V_crit/(V*self.dy*self.dy) - 2.*np.sum(dhistogram_dt*Dr_dt,self.radius
            *self.radius)*np.pi*V_crit/(V*self.dy*self.dy)
        ...
        dEpsOxide_dt = DN_Dt*V_crit/self.V_host + 2.*np.sum(dhistogram_dt*Dr_dt,self.radius
            *self.radius)*np.pi*V_crit/self.V_host
